 ...
 */
function HomeContent(props: DarkModeProps): React.ReactElement {
  ...
  const containers: (items: {}[]) => React.ReactElement[] = (items: {}[]): React.ReactElement[] =>
    items?.map(
      (item: ContainerItem): React.ReactElement => (
        <CSSTransition
          key={item.episode_id}
  ...
  const pages: React.ReactElement[] = episodes?.map(
    (items: [][], indexs: number): React.ReactElement => (
      ...
      >
        {items.length ? <TransitionGroup component={null}>{containers(items)}</TransitionGroup> : noUpdate}
      </div>
    ),
  )
